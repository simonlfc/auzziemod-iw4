...
aim_loop()
{
	...
	if ( isDefined( self.bot.jav_loc ) && !usingRemote )
	{
		...
		if ( curweap == "javelin_mp" && get_bot_var( "core_fire" ) )
	...
	if ( isDefined( self.bot.target ) && isDefined( self.bot.target.entity ) )
	{
		...
		if ( no_trace_time <= no_trace_look_time )
		{
			...
			if ( no_trace_time && ( !isDefined( self.bot.after_target ) || self.bot.after_target != target ) )
			{
				if ( no_trace_time > no_trace_ads_time && !usingRemote )
				{
					if ( isplay )
					{
						...
						if ( !self.bot.isfraggingafter && !self.bot.issmokingafter && get_bot_var( "core_grenade" ) )
						{
			...
			if ( trace_time )
			{
				...
				if ( ( isplay || target.classname == "misc_turret" ) && !self.bot.isknifingafter && conedot > 0.9 && dist < knifeDist && trace_time > reaction_time && !usingRemote && get_bot_var( "core_melee" ) )
				{
				...
				{
					if ( ( !canADS || adsAmount >= 1.0 || self InLastStand() || self GetStance() == "prone" ) && ( conedot > 0.99 || dist < level.bots_maxKnifeDistance ) && get_bot_var( "core_fire" ) )
						self botFire( curweap );
	...
	if ( isDefined( self.bot.after_target ) )
	{
		...
		if ( ( !canADS || adsAmount >= 1.0 || self InLastStand() || self GetStance() == "prone" ) && ( conedot > 0.95 || dist < level.bots_maxKnifeDistance ) && get_bot_var( "core_fire" ) )
			self botFire( curweap );
...
bot_lookat( pos, time, vel, doAimPredict )
{
	...
	if ( level.gameEnded || !gameFlag( "prematch_done" ) || self.bot.isfrozen || !get_bot_var( "core_aim" ) )
		return;
