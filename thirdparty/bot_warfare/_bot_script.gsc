...
init_new_bot()
{
	...
	self setCardTitle( tableLookupByRow( "mp/cardTitleTable.csv", randomInt( 570 ), 0 ) );
	self setCardIcon( tableLookupByRow( "mp/cardIconTable.csv", randomInt( 298 ), 0 ) );
...
bot_get_prestige()
{
	p_dvar = get_bot_var( "profile_prestige" );
	p = 0;
...
{
	rank = 1;
	rank_dvar = get_bot_var( "profile_rank" );
...
teamWatch()
{
	...
	for ( ;; )
	{
		...
		if ( self.team != "axis" || self.team != "allies" )
			self notify( "menuresponse", game["menu_team"], get_bot_var( "manage_team" ) );
...
set_diff()
{
	skill_var = get_bot_var( "skill_level" );
	...
	switch ( skill_var )
	{
		...
		default:
			self.pers["bots"]["skill"]["base"] = skill_var;
			break;
	}
}
...
start_bot_threads()
{
	...
	// script targeting
	if ( get_bot_var( "core_target_other" ) )
	{
